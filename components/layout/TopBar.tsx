...
export function TopBar({ title }: { title: string }) {
  return (
    <header className="h-14 flex-shrink-0 flex items-center justify-between px-6 bg-surface border-b border-border">
      ...
      <button className="flex items-center gap-2.5 px-4 py-2 rounded-xl border border-border-2 bg-surface-2 hover:border-accent transition-all text-sm text-text">
        <Command size={14} className="text-accent" />
        <span>AI Command Center</span>
        <span className="text-[11px] text-faint border border-border px-1.5 py-0.5 rounded">⌘K</span>
      </button>
      <div className="flex items-center gap-2">
        <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-accent hover:bg-accent-2 text-white text-sm font-medium transition-all">
          <Zap size={14} /><span>Quick Action</span>
        </button>
        <button className="relative w-9 h-9 rounded-xl border border-border-2 bg-surface-2 flex items-center justify-center text-muted hover:text-text transition-all">
          <Bell size={15} />
