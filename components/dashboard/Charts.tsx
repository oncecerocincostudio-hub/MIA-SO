...
interface ChartProps { data: Record<string,string|number>[]; dataKey: string; xKey: string; color?: string; title: string; height?: number }

export function AreaChartCard({ data, dataKey, xKey, color='#7c6af5', title, height=220 }: ChartProps) {
  return (
...
export function BarChartCard({ data, dataKey, xKey, color='#22c55e', title, height=220 }: ChartProps) {
  return (
    <div className="bg-surface-2 border border-border rounded-xl p-5">
      <div className="flex justify-between items-center mb-5">
        <span className="text-sm font-medium text-text">{title}</span>
